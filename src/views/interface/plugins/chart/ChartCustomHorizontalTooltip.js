...
const ChartCustomHorizontalTooltip = (props) => {
  ...
  const ExternalTooltip = React.useCallback(({ chart, tooltip }) => {
    ...
    if (tooltipRef.current.opacity === 0) {
     ...
     const left = `${positionX + (tooltip.dataPoints != undefined && tooltip.dataPoints != null && tooltip.dataPoints.length ? tooltip.dataPoints[0].element.x : 0) - 75}px`;
