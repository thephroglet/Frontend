  ...
  if (selectedChat !== null) {
    ...
    return (
      <div className="d-flex flex-column h-100 w-100" id="chatMode">
                ...
                >
                  {selectedChat.messages &&
                    selectedChat.messages.map((message, mIndex) => {
                      const { text, time, type, attachments, name, thumb } = message;
                      if (type === 'response') {
                        return (
                          <RespondContentContainer key={`message${mIndex}`} message={message} user={{ name: name, thumb: thumb }}>
                            <>
                              {text !== '' && <RespondContentText text={text} time={time} name={name} />}
                      ...
                      return (
                        <MessageContentContainer key={`message${mIndex}`} message={message} user={{ name: name, thumb: thumb }}>
                          <>
                            {text !== '' && <MessageContentText text={text} time={time} thumb={thumb} />}
        ...
        <ChatInput sendMessage={props.sendMessage} chat={props.chat} />
      </div>
    );
  }
  return <></>;
